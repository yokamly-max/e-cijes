                        ...
                        <tbody>
                            @foreach($plans as $plan)
                                <tr>
                                    ...
                                    <td class="px-3 py-2">
                                        <form method="POST" action="{{ route('platform.plan.toggleSpotlight') }}">
                                            @csrf
                                    ...
                                    <td class="px-3 py-2">
                                        <form method="POST" action="{{ route('platform.plan.toggleEtat') }}">
                                            @csrf
                                            <input type="hidden" name="id" value="{{ $plan->id }}">
                                            <button type="submit" class="btn btn-secondary btn-sm">
                                                {{ $plan->etat ? '✅' : '❌' }}
                                    ...
                                    <!-- <td class="px-3 py-2">{{ $plan->created_at }}</td> -->
                                    <!-- <td class="px-3 py-2">{{ $plan->updated_at }}</td> -->
                                    <td class="px-3 py-2 text-end">
                                        <a href="{{ route('platform.plan.show', $plan->id) }}" class="btn btn-info btn-sm">
                                            🔍 Détail
                                        </a>
                                        
                                        <a href="{{ route('platform.plan.edit', $plan->id) }}" class="btn btn-warning btn-sm">
                                            ✏️ Modifier
                                        </a>
                                        
                                        <form method="POST" action="{{ route('platform.plan.delete') }}" style="display:inline-block">
                                            @csrf
