                    ...
                    <table id="datatable" class="table table-bordered table-striped w-full text-sm">
                        ...
                        <tbody>
                            @foreach($quartiers as $quartier)
                                <tr>
                                    <td class="px-3 py-2">{{ $quartier->id }}</td>
                                    <td class="px-3 py-2">{{ $quartier->nom }}</td>
                                    <td>
                                        @if ($quartier->commune_id > 0)
                                            {{ $quartier->commune->nom ?? '' }}
                                        @endif
                                    </td>
                                    <td class="px-3 py-2">
                                        <form method="POST" action="{{ route('platform.quartier.toggleEtat') }}">
                                            @csrf
                                    ...
                                    <!-- <td class="px-3 py-2">{{ $quartier->created_at }}</td> -->
                                    <!-- <td class="px-3 py-2">{{ $quartier->updated_at }}</td> -->
                                    <td class="px-3 py-2 text-end">
                                        <a href="{{ route('platform.quartier.show', $quartier->id) }}" class="btn btn-info btn-sm">
                                            🔍 Détail
                                        </a>
                                    </td>
                                    <td class="px-3 py-2 text-end">
                                        <a href="{{ route('platform.quartier.edit', $quartier->id) }}" class="btn btn-warning btn-sm">
                                            ✏️ Modifier
                                        </a>
                                    </td>
                                    <td class="px-3 py-2 text-end">
                                        <form method="POST" action="{{ route('platform.quartier.delete') }}" style="display:inline-block">
                                            @csrf
                                            <input type="hidden" name="quartier" value="{{ $quartier->id }}">
